...
// Ensure role is available for templates (sidebar depends on it)
$role = isset($_SESSION['role']) ? htmlspecialchars($_SESSION['role']) : 'employee';
...
if (isset($_GET['quotation_id'])) {
    ...
    $stmt_sql = "SELECT qi.item, q.quotation_no, q.mode_of_payment, qi.description, qi.qty, qi.unit_price, qi.total 
                           FROM quotation_items qi 
                           JOIN quotations q ON qi.quotation_id = q.id 
                           WHERE qi.quotation_id = ?";
    $stmt = $conn->prepare($stmt_sql);
    if ($stmt === false) {
        // Log prepare error and fall back to a simple query (non-prepared) as a best-effort
        error_log("Prepare failed for quotation prefill: " . $conn->error . " SQL: " . $stmt_sql);
        $qid = (int)$quotation_id;
        $fallback_sql = "SELECT qi.item, q.quotation_no, q.mode_of_payment, qi.description, qi.qty, qi.unit_price, qi.total 
                         FROM quotation_items qi JOIN quotations q ON qi.quotation_id = q.id WHERE qi.quotation_id = $qid";
        if ($res = $conn->query($fallback_sql)) {
            ...
            error_log("Fallback query failed for quotation prefill: " . $conn->error . " SQL: " . $fallback_sql);
        }
    } else {
        if (!$stmt->bind_param("i", $quotation_id)) {
            error_log("bind_param failed: " . $stmt->error);
        } elseif (!$stmt->execute()) {
            error_log("Execute failed for quotation prefill: " . $stmt->error);
        } else {
