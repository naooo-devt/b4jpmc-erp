...
require_once 'calendar_holidays.php';
...
foreach ($calendar_notes as $note) {
    $calendar_events[] = [
        ...
        'start' => $note['date'],
        'allDay' => true,
        'backgroundColor' => '#3b82f6', // Blue for regular notes
        'borderColor' => '#2563eb',
    ];
}

// Add holidays to calendar events with different styling
foreach ($philippine_holidays as $date => $holiday) { 
    // Also add the holiday as a regular event so it shows in the list
    $calendar_events[] = [
...
if ($_POST['action'] === 'add_note') {
    ...
    $stmt = $conn->prepare("INSERT INTO calendar_schedule_note_history (note_date, note_text, user, created_at, updated_at) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $note_date, $note_text, $username, $now, $now);
    $stmt->execute();
    
    // Log the action with timestamp
    $log_text = "[$now] Added note for date: $note_date";
    $log_note_text = $log_text . " (Note: $note_text)";
    $stmt_log = $conn->prepare("INSERT INTO calendar_schedule_note_history (note_date, note_text, user, created_at) VALUES (?, ?, 'System', ?)");
    $stmt_log->bind_param("sss", $now, $log_note_text, $now);
    $stmt_log->execute();
}
...
elseif ($_POST['action'] === 'edit_note') {
    ...
    $stmt = $conn->prepare("UPDATE calendar_schedule_note_history SET note_date = ?, note_text = ?, updated_at = ? WHERE id = ?");
    $stmt->bind_param("sssi", $note_date, $note_text, $now, $note_id);
    $stmt->execute();
    
    // Log the action with timestamp
    $log_text = "[$now] Edited note ID: $note_id";
    $log_note_text = $log_text . " (New note: $note_text)";
    $stmt_log = $conn->prepare("INSERT INTO calendar_schedule_note_history (note_date, note_text, user, created_at) VALUES (?, ?, 'System', ?)");
    $stmt_log->bind_param("sss", $now, $log_note_text, $now);
    $stmt_log->execute();
...
elseif ($_POST['action'] === 'delete_note') {
    ...
    $stmt = $conn->prepare("UPDATE calendar_schedule_note_history SET deleted_at = ? WHERE id = ?");
    $stmt->bind_param("si", $now, $note_id);
    $stmt->execute();
    
    // Log the action with timestamp
    $log_text = "[$now] Deleted note ID: $note_id";
    $log_note_text = $log_text . " (Original note: {$note['note_text']})";
    $stmt_log = $conn->prepare("INSERT INTO calendar_schedule_note_history (note_date, note_text, user, created_at) VALUES (?, ?, 'System', ?)");
    $stmt_log->bind_param("sss", $now, $log_note_text, $now);
    $stmt_log->execute();
...
<body>
 ...
 if (file_exists($sidebar_path)) {
     ...
     echo '<div class="sidebar" style="width:220px;position:fixed;left:0;top:0;height:100vh;background:#0ea5e9;padding:16px;color:#fff;">';
     echo '<h3>Sidebar missing</h3>';
     echo '<p style="font-size:0.85rem;">Please restore <code>sidebar.php</code></p>';
     echo '</div>';
 }
 ?>
                                        ...
                                        if ($raw_materials_result && $raw_materials_result->num_rows > 0) {
                                            ...
                                            while ($row = $raw_materials_result->fetch_assoc()) {
                                                ...
                                                echo "<td data-label='Actions' class='table-actions'>
                                                        ...
                                                        <button class='btn btn-danger delete-raw-btn' onclick=\"deleteRawMaterial('" . htmlspecialchars($row['id']) . "')\"><i class='fas fa-trash'></i> <span class='action-text'>Delete</span></button>
                                                      </td>";
                                                echo "</tr>";
                                            }
                                        } else {
    ...
    <!-- Delete Raw Material Confirmation Modal -->
<div id="deleteRawMaterialModal" class="modal-overlay" style="display:none;">
    <div class="modal-content">
        <span class="close-modal" onclick="closeDeleteRawMaterialModal()">&times;</span>
        <form id="deleteRawMaterialForm" method="POST" action="delete_material.php">
            <input type="hidden" name="id" id="deleteRawMaterialId">
            <div class="form-header">
            ...
            <div style="margin-top: 25px; display: flex; gap: 12px; justify-content: flex-end;">
                ...
                <button class="btn btn-outline simple-btn" type="button" onclick="closeDeleteRawMaterialModal()">
                    <i class="fas fa-times"></i> Cancel
                </button>
            </div>
        </form>
    ...
    <div id="viewNoteModal" class="modal-overlay" style="display:none;">
        <div class="modal-content">
            <span class="close-modal" onclick="closeViewNoteModal()">&times;</span>
            <div class="form-section" style="box-shadow:none; border:none;">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label"><i class="fas fa-calendar-day"></i> Date</label>
                        <p id="viewNoteDate" class="form-input" style="border:none; background:none;"></p>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label"><i class="fas fa-user"></i> Created By</label>
                        <p id="viewNoteUser" class="form-input" style="border:none; background:none;"></p>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label"><i class="fas fa-sticky-note"></i> Note Content</label>
                        <p id="viewNoteContent" class="form-input" style="border:none; background:none; white-space: pre-line;"></p>
                    </div>
                </div>
                <div style="margin-top: 25px; display: flex; gap: 12px; justify-content: flex-end;">
                    <button class="btn btn-outline simple-btn" type="button" onclick="closeViewNoteModal()">
...
document.addEventListener('DOMContentLoaded', function() {
        // Create a mapping of note IDs to note details
        const noteDetailsMap = {};
        <?php foreach ($calendar_notes as $note): ?>
        noteDetailsMap[<?php echo $note['id']; ?>] = {
            id: <?php echo $note['id']; ?>,
            date: "<?php echo date('M d, Y', strtotime($note['date'])); ?>",
            text: `<?php echo addslashes(str_replace(["\r\n", "\r", "\n"], '\n', $note['text'])); ?>`,
            user: "<?php echo addslashes($note['user']); ?>"
        };
        <?php endforeach; ?>

        // Initialize FullCalendar with event click handling
        if (document.getElementById('calendar')) {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventDidMount: function(info) {
                    ...
                    if (info.event.title.includes('Holiday:')) {
                ...
                eventClick: function(info) {
                    ...
                    if (note) {
                        ...
                        document.getElementById('viewNoteUser').textContent = note.user;
                        document.getElementById('viewNoteContent').textContent = note.text;
                    ...
                    document.addEventListener('keydown', function(event) {
                        if (event.key === 'Escape') {
                            ...
                            if (viewNoteModal && viewNoteModal.style.display === 'flex') {
                                closeViewNoteModal();
                            }
                        }
                    });
    ...
    function deleteRawMaterial(id) {
        if (confirm('Are you sure you want to delete this raw material?')) {
            ...
            form.method = 'POST';
            form.action = 'delete_raw_material.php';
            
            // Add the ID as a hidden input
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('click', function(e) {
            let viewBtn = e.target.closest('.view-raw-btn');
            let editBtn = e.target.closest('.edit-raw-btn');
            let deleteBtn = e.target.closest('.delete-raw-btn');
            if (viewBtn) {
                const id = viewBtn.getAttribute('data-raw-id');
                openViewRawMaterialModal(id);
            }
            if (editBtn) {
                const id = editBtn.getAttribute('data-raw-id');
                openEditRawMaterialModal(id);
            }
            if (deleteBtn) {
                const id = deleteBtn.getAttribute('data-raw-id');
                openDeleteRawMaterialModal(id);
            }
        });

        function openDeleteRawMaterialModal(id) {
            document.getElementById('deleteRawMaterialId').value = id;
            document.getElementById('deleteRawMaterialModal').style.display = 'flex';
        }

        function closeDeleteRawMaterialModal() {
            document.getElementById('deleteRawMaterialModal').style.display = 'none';
        }

        // Search functionality - UPDATED to only search in Material Name column (2nd column)
        ...
        if (searchInput) {
            searchInput.addEventListener('input', function() {
                ...
                for (let row of rows) {
                    // Get the Material Name cell (2nd column, index 1)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Variable-date holidays (add calculations here if needed)
    // Example: const easter = calculateEaster(year);
